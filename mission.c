...
PUU32 MissionSetAttr( PULID _Object, PULID _Class, void* _pData, PUU32 
_Attr, PUU32 _Val );
PUU32 MissionParse( PULID _Object, MissionClassData* _pData, PUU8* 
_pMissionData );
...
PUU32 SetAndSearch( PUU8* _pSrcString, PULID _TextEntry, PULID _List )
{
	...
	while( Record )
	{
		if( ( pString = ( PUU8* )puDoMethod( _List, PUM_TABLE_GETFIELDVAL, Record, 
0 ) ) && *pString )
		{
...
PUA_CHECKBOX_CHECKED) );
			...
			else // location
			{
				if( LocationMatch( TmpItemName, pString ) )
				{
					if( !g_BuyingAgentCount )
						puSetAttribute( _TextEntry, PUA_TEXTENTRY_HILIGHT, puGetAttribute( 
puGetObjectFromCollection(g_pCol, CS_HIGHLIGHTLOC_CB), PUA_CHECKBOX_CHECKED) 
);
					return TRUE;
				}
			}
...
SearchCoordYFrom[20], SearchCoordYTo[20];
	...
	if ( ItemMatch( Name, Search ) ) {
		...
		if( x > 0 && y > 0 && xfrom > 0 && yfrom > 0 ) { // carry on only if we have mission location
...
PUU8 a_xPNGSig[LEN_PNGSIG] = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 
0x0A};
...
fpDebug);
	...
	if (uncompress(a_xPNGImage, &lPNGImageLen, a_xPNGData, lPNGDataLen) != 
Z_OK)
		goto GetAOIconData_Exit_Fail;
...
//ibender: this is just a test to see if I can commit
